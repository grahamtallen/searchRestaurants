...
const fs = require('fs')
const RESTAURANTS_ALL_PATH =
    'csv/restaurants.csv'
const DATA_STORES_PATH =
    'data-stores/'

const main = async () => {
    // todo join cusine
    const restaurantsAll = await csvtojson().fromFile(
        RESTAURANTS_ALL_PATH
    )

    saveToDataStore(
        restaurantsAll,
        'restaurants-all.json'
    )
    saveToDataStore(
        restaurantsAll.sort(
            (a, b) =>
    ...
    let byName = {}
    restaurantsAll.forEach(
        restaurant => {
            const {
                name,
            } = restaurant
            const nameLowerCaseNoSpaces = name
                .toLowerCase()
                .replace(
                    /\s/g,
                    ''
    ...
    restaurantsAll.forEach(
        restaurant => {
            ...
            switch (
                ...
                case '5':
                    FiveStarRestaurants.push(
                        restaurant
                    )
                    break
                case '4':
                    FourStarRestaurants.push(
                        restaurant
                    )
                    break
                case '3':
                    ThreeStarRestaurants.push(
                        restaurant
                    )
                    break
                case '2':
                    TwoStarRestaurants.push(
                        restaurant
                    )
                    break
                case '1':
                    OneStarRestaurants.push(
                        restaurant
                    )
                    break
                default:
                    throw new Error(
                        'Unhandled restaurant rating: ' +
                            customer_rating
                    )
            }
        }
    ...
    saveToDataStore(
        completeBucketedRestaurants,
        'bucketed-by-rating.json'
    )

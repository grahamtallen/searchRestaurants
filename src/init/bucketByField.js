...
const bucketArrayByColumn = (array, column, order = 'desc') => {
    ...
    array.forEach((element) => {
        ...
        if (!bucketForThisValue) {
            buckets[numValue] = {
                data: [element],
                range: order === 'desc' ? `0 through ${numValue}` : `${numValue} through 0`
            }
...
const resolveBuckets = (newItemToAddToBucket, buckets, searchIndex, max, order) => {
        ...
        if (order === 'desc') {
            ...
            for (let i = max; i > searchIndex; i--) {
                ...
                if (bucketExistsAtThisHigherIndex) {
                    bucketExistsAtThisHigherIndex.data.unshift(newItemToAddToBucket)
                }
            }
        } else if (order = 'asc') {
...
const bucketsByRating = bucketArrayByColumn(sortedByDistance.reverse(), 'customer_rating', 'asc'); // reverse very important here for the type of ordering we're trying to do in the next function
